...
"""
def solution(A):
    max_passes = 1000000000
    # Represents total passes for a single car traveling east
    total_passes = 0
    total_going_east = 0
    for i in A:
        going_east = i == 0
        going_west = not going_east

        if going_east:
            total_going_east += 1

        if going_west:
            # Total pass cars increases as the number of cars
            # traveling in the same direction increases
            total_passes += 1 * total_going_east
