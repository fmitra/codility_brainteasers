...
def solution(S, P, Q):
    ...
    # We'll increment the occurance of each letter here and use the indexes
    # of the array to learn if a letter occured within that range
    prefix_sum_array = [
        {
            'A':0,
            'C':0,
    ...
    solutions = []
    query_length = len(P)
    letters_length = len(S)
    
    for i in xrange(letters_length):
        next_dict = prefix_sum_array[i].copy()
        # increment the count for letter occurance
        next_dict[S[i]] += 1
        prefix_sum_array.append(next_dict)
    for i in xrange(query_length):
        start = P[i]
        end = Q[i]
        end_dict = prefix_sum_array[end + 1].copy()
        for key, count in prefix_sum_array[start].iteritems():
            end_dict[key] -= count
        genes = [key for key, count in end_dict.iteritems() if count > 0]
        genes.sort()
        solutions.append(dna_map[genes[0]])
