...
You are given two non-empty zero-indexed arrays A and B c
onsisting of N integers. Arrays A and B represent N voracious
fish in a river, ordered downstream along the flow of the river.

The fish are numbered from 0 to N − 1. If P and Q are two fish
and P < Q, then fish P is initially upstream of fish Q. Initially,
each fish has a unique position.

Fish number P is represented by A[P] and B[P]. Array A contains
the sizes of the fish. All its elements are unique. Array B
...
More precisely, we say that two fish P and Q meet each other when
P < Q, B[P] = 1 and B[Q] = 0, and there are no living fish between
...
func Solution(A []int, B []int) int {
    ...
    for i := 0; i < ln; i++ {
        ...
        if direction == 1 {
            upstream = append(upstream, size)
        } else {
            downstream++
            for len(upstream) > 0 {
                upstream_fish := upstream[len(upstream)-1]
                upstream = upstream[:len(upstream)-1]
                if size < upstream_fish {
                    upstream = append(upstream, upstream_fish)
                    downstream--
                    break
                }
            }
